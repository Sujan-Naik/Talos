...
public:
    ...
    Q_INVOKABLE void setTtsVoice(
        const QString &voice
    );

    Q_INVOKABLE void onTtsToggled(
        bool enabled
    );
...
private slots:
    ...
    void handleTtsVoicesChanged(
        const QStringList &voices
    );

    void handleTtsVoiceChanged(
        const QString &voice
    );
